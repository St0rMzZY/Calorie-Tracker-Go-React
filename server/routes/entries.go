...
import (
	...
	"github.com/St0rMzZY/go-react-calorie-tracker/models"
...
var entryCollection *mongo.Collection = OpenCollection(Client, "context")
...
func AddEntry(c *gin.Context) {
	...
	if insertErr != nil {
		msg := fmt.Sprintf("orderitem was not created")
...
func GetEntriesByIngregient(c *gin.Context) {
...
func UpdateIngregient(c *gin.Context) {
	entryID := c.Params.ByName("id")
	docID, _ := primitive.ObjectIDFromHex(entryID)
	var ctx, cancel = context.WithTimeout(context.Background(), 100*time.Second)
	...
	result, err := entryCollection.UpdateOne(ctx, bson.M{"_id": docID},
		bson.D{{"&set", bson.D{{"ongredients", ingredient.Ingredients}}}},
	)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		fmt.Println(err)
